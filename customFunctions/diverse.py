...
def update_guild_setting(guild_id: int, settings: dict) -> None:
    ...
    :return None.
    :raises UndefinedColumn if setting does not exist
    :param guild_id (int) ID of the Guild
    ...
    if settings != {}:
        ...
        setting_names_excluded = []
        for setting_name in setting_names:
            setting_names_excluded.append('{0} = excluded.{0}'.format(setting_name))

        q = "INSERT INTO guild_settings (guild_id, {rows}) VALUES ({values})" \
            "ON CONFLICT (guild_id) DO UPDATE SET {excluded};"\
            .format(
                rows = ', '.join(setting_names),
                values = ', '.join(['$%i' % i for i in range(1, len(setting_values)+1)]),
                excluded = ', '.join(setting_names_excluded)
            )

        shortsql.sync_save_write(q, setting_values, 1)

    else:
        shortsql.sync_save_write("insert into guild_settings (guild_id) values (%s) ON CONFLICT DO NOTHING;",
                                 (guild_id,), 1)


async def async_get_guild_setting(guild_id: int, settings: Union[str, Iterable]):
    """ Search the Database for Guild-Settings. Creates Row if Guild is not in Settings.
    :return Setting from the DB.
    :raises psycopg2.errors.UndefinedColumn if setting does not exist
    :param guild_id (int) ID of the Guild
    ...
    q = f"SELECT {', '.join(settings)} FROM guild_settings WHERE guild_id = $1;"
    r = await shortsql.save_read(q, (guild_id,))

    if len(r) == 0:
        await shortsql.save_write("INSERT INTO guild_settings (guild_id) VALUES ($1);", (guild_id,), 1)
        r = await shortsql.save_read(q, (guild_id.__str__(),))
...
async def async_update_guild_setting(guild_id: int, settings: dict) -> None:
    """ Update a Setting in the Database. If Guild does not exist, make entry.
    :return None.
    :raises psycopg2.errors.UndefinedColumn if setting does not exist
    :param guild_id (int) ID of the Guild
    ...
    if settings != {}:
        ...
        await shortsql.save_write(q, setting_values, 1)

    else:
        await shortsql.save_write("insert into guild_settings (guild_id) values (%s) ON CONFLICT DO NOTHING;",
                                  (guild_id,), 1)
